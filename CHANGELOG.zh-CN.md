...
- **Cursor 配额刷新会自动重试瞬时失败**：单账号刷新在超时、限流、5xx，或 token 刷新失败后仍用旧会话拉配额时，最多再试 3 次。账号不存在、JWT 解析不出 WorkOS user id 这类结构性错误不重试。

### 变更

- **Cursor 导入后并行刷新配额**：Token / JSON 或文件导入成功后，新账号最多 5 路同时拉配额，整批结束后只重载一次列表。
