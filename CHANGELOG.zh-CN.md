...
- **Cursor 切号前把网页 token 换成 IDE session**：`type=web` 的网页 JWT 能打开套餐页，但 Cursor 3.21 的聊天会报未登录。切号写库前用网页 cookie 换成 `type=session`；换票失败则停住，不再把网页 token 写进本地登录库。已经是 session 的账号不重复换票。
...
- **Cursor 单账号无感切号**：账号页工具栏增加无感开关。开启后给 Cursor 的 workbench 打补丁，切号不再重启；关闭时仍先退出 Cursor，再按固定登录字段写回并启动。不改应用多开。写入安装目录被系统拦住时，打开「App 管理」，不再要求管理员密码。
...
- **Cursor 列表可按创建时间拖拽排序**：仅在列表视图且排序为「按创建时间」时，可拖拽调整账号顺序；顺序通过改写创建时间落盘。当前应用账号始终置顶，不参与拖拽。
...
- **Cursor 导入不再产生重复账号**：`grok|user_01...` 与 `user_01...` 视为同一身份，同一邮箱会在下次加载列表时合并。

### 变更

- **Cursor 展示 Bot 额度**：账号列表、卡片、托盘和 macOS 菜单在有数据时显示 Bot 用量。
...
- **恢复 Cursor 工具栏导出**：支持文本行（`email----user_id::jwt`）和 JSON，可复制或下载。
- **无标签单元格不再显示占位符**：标签列没有标签时留空，不再显示 `—`。
...
- **Cursor 账号恢复标签编辑**：卡片和列表操作栏重新提供标签按钮，可添加、修改账号标签。
- **Cursor 列表标签单独成列**：有标签时在邮箱和订阅之间展示标签列；全部账号都没有标签时自动隐藏该列。
- **隐藏 Cursor 账号导出入口**：卡片、列表和工具栏不再提供导出，避免只藏按钮仍能触发导出。
...
- **Cursor 账号卡片显示添加时间**：卡片底部展示账号添加日期和时间，便于区分账号。
- **Cursor 列表额度可直接点击刷新**：额度单元格合并为可点击区域，点击即可刷新该账号配额。
- **Cursor 切号身份快照更完整**：切号时按身份相关键读写 VSCDB，并清理过期账号缓存，减少切号后残留旧登录状态。
