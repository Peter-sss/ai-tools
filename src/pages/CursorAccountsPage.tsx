...
const CURSOR_TOKEN_LINE_EXAMPLE = `user@example.com----user_01EXAMPLE00000000000000::eyJhbGciOiJIUzI1NiIs...`;
const CURSOR_TOKEN_BATCH_EXAMPLE = `user@example.com----user_01EXAMPLE00000000000000::eyJhbGciOiJIUzI1NiIs...
other@example.com----user_01EXAMPLE00000000000001::eyJhbGciOiJIUzI1NiIs...`;
const CURSOR_TOKEN_JSON_EXAMPLE = `[
...
function extractWorkosUserIdFromJwt(jwt: string): string | null {
  try {
    ...
    return userId.startsWith("user_") ? userId : userId || null;
  } catch {
    return null;
  }
}

function formatCursorTokenLine(account: CursorAccount): string | null {
  const email = (account.email || "").trim() || "unknown";
  const token = (account.access_token || "").trim();
  if (!token) return null;
  let authId = (account.auth_id || "").trim();
  if (authId.includes("|")) {
...
function formatResetDaysLeft(
  ...
  const msLeft = resetTs * 1000 - Date.now();
...
export function CursorAccountsPage() {
  ...
  const [filterTypes, setFilterTypes] = useState<string[]>(() =>
    readAccountsOverviewFilterPersistenceEnabled(
      CURSOR_FILTER_PERSISTENCE_SCOPE,
    )
      ? readAccountsOverviewFilterStringArray(
          CURSOR_FILTER_PERSISTENCE_SCOPE,
          FILTER_TYPES_FIELD,
  ...
  const page = useProviderAccountsPage<CursorAccount>({
    ...
    oauthService: {
      ...
      completeLogin: (loginId: string) =>
        cursorService.completeCursorOAuthLogin(loginId),
      cancelLogin: (loginId?: string) =>
        cursorService.cancelCursorOAuthLogin(loginId),
    },
    dataService: {
  ...
  const resolveTotalQuota = useCallback((account: CursorAccount) => {
  ...
  const resolveAutoQuota = useCallback((account: CursorAccount) => {
    const usage = getCursorUsage(account);
  ...
  const resolveApiQuota = useCallback((account: CursorAccount) => {
    const usage = getCursorUsage(account);
  ...
  const resolveOnDemandQuota = useCallback(
    (account: CursorAccount) => {
      ...
      const rawPct =
        onDemand.limitCents && onDemand.limitCents > 0
  ...
  const resolveResetTime = useCallback((account: CursorAccount) => {
    const usage = getCursorUsage(account);
  ...
  const handleCopyAccountTokenLine = useCallback(
    async (account: CursorAccount, event?: SyntheticEvent) => {
      event?.preventDefault();
      event?.stopPropagation();
      const showCopyToast = (text: string, tone: "success" | "error") => {
        if (copyToastTimerRef.current != null) {
          window.clearTimeout(copyToastTimerRef.current);
        }
        setCopyToast({ text, tone });
      ...
      const emailLabel =
        (account.email || "").trim() ||
        resolveDisplayEmail(account) ||
        account.id;
      const line = formatCursorTokenLine(account);
      if (!line) {
        showCopyToast(
          t("cursor.copyTokenLine.missing", {
            email: emailLabel,
            defaultValue: "缺少 access token，无法复制 {{email}}",
          }),
          "error",
        );
        return;
      ...
      try {
        ...
        showCopyToast(
          t("cursor.copyTokenLine.success", {
            email: emailLabel,
            defaultValue: "已复制 {{email}} 的 Token 行",
          }),
          "success",
        );
      } catch (error) {
        showCopyToast(String(error), "error");
      }
    },
  ...
  const renderPlanBadge = useCallback(
    (account: CursorAccount, resetTs: number | null) => {
      const planLabel = resolvePlanLabel(account);
      const daysLeft = formatResetDaysLeft(resetTs);
      return (
        <span
          className={`tier-badge cursor-plan-badge ${resolvePlanBadgeClass(account)}`}
        >
          <span className="cursor-plan-badge-label">{planLabel}</span>
          {daysLeft ? (
            <>
              <span className="cursor-plan-badge-sep" aria-hidden="true" />
              <span className="cursor-plan-badge-days">{daysLeft}</span>
            </>
          ) : null}
        </span>
      );
    },
    [resolvePlanBadgeClass, resolvePlanLabel],
  );
  ...
  const compareAccountsBySort = useCallback(
    (a: CursorAccount, b: CursorAccount) => {
      ...
      return (
        ...
        >
          <div className="card-top">
            ...
            <div
              className="cursor-account-copy-header"
              role="button"
              tabIndex={0}
              title={t(
                "cursor.copyTokenLine.hint",
                "点击复制 email----user_id::token",
              )}
              onClick={(event) =>
                void handleCopyAccountTokenLine(account, event)
              }
              onKeyDown={(event) => {
            ...
            >
              ...
              {renderPlanBadge(account, resetTs)}
            </div>
          </div>
              ...
              <>
                <div className="quota-item windsurf-credit-item">
                  <div className="quota-header">
                    ...
                    <span className={`quota-pct ${total.quotaClass}`}>
                      {total.valueText}
                    </span>
                  </div>
                  {total.costText && (
                  ...
                  <div className="quota-bar-track">
                    <div
                      className={`quota-bar ${total.quotaClass}`}
                ...
                <div className="quota-item windsurf-credit-item">
                  <div className="quota-header">
                    ...
                    <span className={`quota-pct ${auto.quotaClass}`}>
                      {auto.valueText}
                    </span>
                  </div>
                  <div className="quota-bar-track">
                    <div
                      className={`quota-bar ${auto.quotaClass}`}
                ...
                <div className="quota-item windsurf-credit-item">
                  <div className="quota-header">
                    ...
                    <span className={`quota-pct ${api.quotaClass}`}>
                      {api.valueText}
                    </span>
                  </div>
                  <div className="quota-bar-track">
                ...
                <div className="quota-item windsurf-credit-item">
                  <div className="quota-header">
                    ...
                    <span className={`quota-pct ${onDemand.quotaClass}`}>
                      {onDemand.valueText}
                    </span>
                  </div>
                  {onDemand.costText && (
                  ...
                  <div className="quota-bar-track">
                    <div
                      className={`quota-bar ${onDemand.quotaClass}`}
                      style={{
    ...
    items.map((account) => {
      ...
      return (
        ...
        >
          ...
          <td>
            <div
              className="account-cell cursor-account-copy-header"
              role="button"
              tabIndex={0}
              title={t(
                "cursor.copyTokenLine.hint",
          ...
          <td>
            <button
              type="button"
              className="cursor-account-copy-plan-btn"
              title={t(
                "cursor.copyTokenLine.hint",
                "点击复制 email----user_id::token",
              )}
  ...
  return (
    <div className="ghcp-accounts-page cursor-accounts-page">
        ...
        <>
            ...
            <div className="toolbar-left">
              ...
              <SingleSelectFilterDropdown
                ...
                options={[
                  ...
                  {
                    value: "credits",
                    label: t("common.shared.sort.credits", "按剩余 Credits"),
                  },
                  {
                    value: "plan_end",
                    label: t(
                      "common.shared.sort.planEnd",
                      "按配额周期结束时间",
                    ),
                  },
                ]}
          ...
          {loading && accounts.length === 0 ? (
            ...
            <div className="empty-state">
              ...
              <p>
                {t(
                  "cursor.empty.description",
                  '点击"添加账号"开始管理您的 Cursor 账号',
                )}
              </p>
              <div
                style={{
                ...
                <div className="modal-body">
                  ...
                  {addTab === "token" && (
                    <div className="add-section">
                      <p className="section-desc">
                        {t(
                          "cursor.token.desc",
                          "粘贴 Cursor Token：支持 JWT、email----user_id::jwt、user_id::jwt，或 JSON。支持多行批量导入。",
                        )}
                      </p>
                      <details className="token-format-collapse">
                        <summary className="token-format-collapse-summary">
                          {t(
                            "cursor.token.formatHint",
                            "必填字段与示例（点击展开）",
                          )}
                        </summary>
                        <div className="token-format">
                          <p className="token-format-required">
                            {t(
                              "cursor.token.formatRequired",
                              "推荐格式：email----user_id::jwt；也支持纯 JWT / user_id::jwt；批量可多行或 JSON 数组",
                            )}
                          </p>
                          <div className="token-format-group">
                            <div className="token-format-label">
                              {t(
                                "cursor.token.lineExample",
                                "单条示例（email----user_id::jwt）",
                              )}
                            </div>
                            <pre className="token-format-code">
                              {CURSOR_TOKEN_LINE_EXAMPLE}
                            </pre>
                          </div>
                          <div className="token-format-group">
                            <div className="token-format-label">
                              {t(
                                "cursor.token.singleExample",
                                "单条示例（JWT）",
                              )}
                            </div>
                            <pre className="token-format-code">
                          ...
                          <div className="token-format-group">
                            <div className="token-format-label">
                              {t(
                                "cursor.token.batchExample",
                                "批量示例（JSON）",
                              )}
                            </div>
                            <pre className="token-format-code">
                              {CURSOR_TOKEN_JSON_EXAMPLE}
                            </pre>
                          </div>
                        </div>
                      </details>
                      <textarea
                        ...
                        placeholder={t(
                          "cursor.token.placeholder",
                          "粘贴 Token / email----user_id::jwt / JSON...",
                        )}
                      />
                      <button
                        className="btn btn-primary btn-full"
                  ...
                  {addTab === "import" && (
                    <div className="add-section">
                      <p className="section-desc">
                        {t(
                          "cursor.import.localDesc",
                          "支持从本机 Cursor 客户端或 JSON 文件导入账号数据。",
                        )}
                      </p>
                      <button
      ...
      {copyToast && (
        <div className="cursor-copy-toast-container" aria-live="polite">
          <div className={`toast cursor-copy-toast ${copyToast.tone}`}>
            {copyToast.text}
          </div>
        </div>
      )}
