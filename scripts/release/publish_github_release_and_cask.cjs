...
const DEFAULT_REPO = 'Peter-sss/ai-tools';
const DEFAULT_CASK_PATH = 'Casks/ai-tools.rb';
const DEFAULT_TARGET = 'universal-apple-darwin';
const DEFAULT_RELEASE_ASSET_PREFIX = 'ai-tools';
...
function printHelp() {
  ...
  console.log('  --repo <owner/repo>     GitHub repo for release (default: Peter-sss/ai-tools)');
  console.log('  --cask <path>           Homebrew cask file path (default: Casks/ai-tools.rb)');
...
function resolveSourceDmgPath(version, options) {
  ...
  const defaultPath = path.resolve(
    ...
    'bundle',
    'dmg',
    `ai-tools_${version}_universal.dmg`
  );
...
function main() {
  ...
  console.log('ai-tools GitHub Release + Homebrew cask publisher');
  ...
  console.log('- 检查 Casks/ai-tools.rb diff');
